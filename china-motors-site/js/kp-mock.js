...
// ============================================================================
//
// СТАТУС: закрыто на бэкенде. GET /api/kp/<id>/ считает оба числа из одной
// разбивки (BackChinaMotors, core/kp_instant.py → build_instant_kp), а
// vehicle.price_kzt в документ не попадает вовсе. Оговорка оставлена как
// описание того, ПОЧЕМУ так сделано: если однажды покажется, что в ячейку
// «Сумма» логичнее подставить цену каталога, здесь написано, чем это
// кончится.
//
...
// ============================================================================
//
// СТАТУС: реализовано. GET /api/kp/<id>/pdf/ отдаёт подписанный документ
// (BackChinaMotors, core/kp_instant.py → build_instant_kp_pdf), а JSON
// приносит на него АБСОЛЮТНУЮ ссылку — сайт и API на разных хостах. Мок
// по-прежнему отдаёт null: он работает ровно тогда, когда бэкенд не
// ответил, и подписанного документа в этот момент взять неоткуда.
//
